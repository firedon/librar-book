...
if(@$_POST['reg']){
	...
	$result=mysql_query($ck_sql,$conn);
	if($re_arr=mysql_fetch_array($result)){
		$um=$re_arr['name'];
...
//修改密码
	...
	mysql_query($sql,$conn) or die ("密码修改失败: ".mysql_error());
	echo "<p align='center'>密码修改成功!</p>";
}
else {
	$sql="delete from user where name='$name'";
	mysql_query($sql,$conn) or die ("用户删除失败: ".mysql_error());
